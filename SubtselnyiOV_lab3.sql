...
--   Task5
WITH ProductsSOUTH ([ProductName]) AS (
SELECT DISTINCT [ProductName]
FROM [NORTHWND].[dbo].[Products] AS PD
JOIN [NORTHWND].[dbo].[Order Details] AS OD ON
	OD.[ProductID] = PD.[ProductID]
JOIN [NORTHWND].[dbo].[Orders] AS ORD ON
	ORD.[OrderID] = OD.[OrderID]
JOIN [NORTHWND].[dbo].[EmployeeTerritories] AS ET ON
	ET.[EmployeeID] = ORD.[EmployeeID]
...
SELECT [ProductName] FROM [NORTHWND].[dbo].[Products]
WHERE [ProductName] != ALL(SELECT * FROM ProductsSOUTH)
